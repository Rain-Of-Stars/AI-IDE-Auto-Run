...
# 开启早期因子注入（可被外部覆盖）
os.environ.setdefault("AIIDE_ENABLE_QT_SCREEN_SCALE_FACTORS", "1")
...
class RefactoredTrayApp(QtWidgets.QSystemTrayIcon):
    ...
    def _on_progress_update(self, message: str):
        ...
        if message and hasattr(self, '_progress_timer'):
            self._update_status("启动中", "进程", message)
            # 重置进度计数器，避免重复消息
            self._progress_counter = 0
    ...
    def _handle_config_save_status(self, request):
        ...
        if status == 'saving':
            ...
            if path:
                ...
                # 显示3秒后恢复工具提示
                QtCore.QTimer.singleShot(3000, lambda: self.setToolTip(f"AI-IDE-Auto-Run - {self._cached_status}"))
        elif status == 'error':
...
def check_single_instance():
    ...
    base_name = "AI_IDE_Auto_Run_Instance"
