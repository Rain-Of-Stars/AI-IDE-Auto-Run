...
class ScannerProcessManager(QObject):
    ...
    def _on_startup_timeout(self):
        ...
        try:
            ...
            if self._startup_attempt < self._max_startup_attempts and self._current_config is not None:
                ...
                try:
                    QtCore.QTimer.singleShot(delay_ms, lambda: self.start_scanning(self._current_config))
                except Exception as e:
                    # 若无事件循环，记录提示并直接同步重试一次（用于单元测试环境）
                    self._logger.debug(f"singleShot失败，直接同步重试: {e}")
                    self.start_scanning(self._current_config)
            else:
                # 用尽重试，报错并停机
    ...
    def _poll_queues(self):
        ...
        try:
            ...
            # 单次轮询时间预算（毫秒），严格限制本方法占用主线程的时间，避免UI卡顿
            MAX_TICK_BUDGET_MS = 8
            ...
            while self._status_queue and processed < MAX_STATUS_PER_TICK:
                try:
                    ...
                    break
                # 时间预算检查
                if (time.perf_counter() - _tick_start) * 1000.0 > MAX_TICK_BUDGET_MS:
                    break
